...
def load_raw_products_to_postgres(data: pl.DataFrame) -> None:
    """
    ...
    Parameters:
    - data (pl.DataFrame): Raw products data from extraction
...
def create_staging_supermarkets_table():
    """
...
def create_prod_supermarkets_table():
    """
...
def load_staging_data_from_raw(raw_table_name: str):
    """
...
def load_prod_data_from_staging():
    """
...
def create_staging_products_table():
    """
...
def create_prod_products_table():
    """
    Create production schema and final table structure for API consumption.
    """
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1 FROM pg_constraint 
            WHERE conname = 'prod_products_name_extracted_date_key'
        ) THEN
            ALTER TABLE prod.products 
            ADD CONSTRAINT prod_products_name_extracted_date_key 
...
def load_staging_products_from_raw(raw_table_name: str):
    """
